...
import "./QuestionarePage.css"
 function QuestionarePage({ onQuestionareComplete }) {
	const questions = [
		...
		{
			questionText: 'How much time do you spend sitting or being inactive during a typical workday?',
			answerOptions: [
				...
				{ answerText: 'Very little sitting, often standing or moving (3 points)', score: 3 },
			],
		},
		{
			questionText: 'How do you commute to work or travel short distances?',
			answerOptions: [
				{ answerText: ' Drive or use public transportation (0 points)',score: 0},
				{ answerText: 'Walk or cycle occasionally (1 point)',score: 1 },
				{ answerText: 'Walk or cycle most of the time (2 points', score: 2 },
				{ answerText: 'Always walk or cycle, no motorized transportation (3 points)', score: 3 },
			],
		},
		{
			questionText: 'How active are you during leisure time (e.g., playing sports, going for walks)?',
			answerOptions: [
				{ answerText: 'Rarely or never (0 points)', score: 0 },
				{ answerText: 'Sometimes, but not regularly (1 point)', score: 1 },
				{ answerText: 'Fairly regularly (2 points)', score: 2 },
				{ answerText: 'Very regularly and enjoy staying active (3 points', score: 3 },
			],
		},
        {
			questionText: 'How often do you engage in activities that involve moderate to vigorous physical effort?',
			answerOptions: [
				{ answerText: 'Rarely or never (0 points)',score: 0 },
				{ answerText: 'Sometimes, but not regularly (1 point)', score: 1 },
				{ answerText: 'Fairly regularly (2 points)', score: 2 },
				{ answerText: 'Very regularly and enjoy staying active (3 points', iscore:3},
			],
		},
	];
	...
	return (
		<div className='QuestionarePage'>
			{showScore ? (
				<div className='score-section'>
				         You scored {totalScore} out of {questions.reduce((total, q) => total + q.score, 0)}
				</div>
			) : (
				<>
                    ...
                    <div className='question-section'>
						...
						{questions[currentQuestion].answerInput ? (
							<input
								type='text'
								value={userAnswers[questions[currentQuestion].answerInput] || ''}
								onChange={(e) => handleAnswerInput(questions[currentQuestion].answerInput, e.target.value)}
							/>
						) : (
